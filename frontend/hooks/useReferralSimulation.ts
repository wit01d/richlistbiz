...
export const SUCCESSOR_THRESHOLD = 13;
...
export interface SimEvent {
  id: string;
  type: 'user_created' | 'deposit' | 'successor' | 'info' | 'fraud_alert';
  message: string;
  timestamp: number;
...
export const createSuccessorScenario = (): SimulationState => {
  ...
  scenarioState.events = [
    ...
    { id: 'evt2', type: 'info', message: 'Carol has 13 depositing recruits - READY for successor!', timestamp: Date.now() - 100 },
    { id: 'evt3', type: 'info', message: 'Xavier has 7 depositing recruits - getting close!', timestamp: Date.now() - 200 },
    { id: 'evt4', type: 'info', message: 'Click Play or add recruits to Carol to trigger successor', timestamp: Date.now() - 300 },
  ];
  ...
  const processDeposit = useCallback((userId: string) => {
    setState(prev => {
      ...
      if (referrerId && referrerId !== SYSTEM_ID && newUsers[referrerId]) {
        ...
        if (
          referrer.depositingRecruits >= SUCCESSOR_THRESHOLD &&
          !referrer.successorNominated &&
          listline.position1 !== SYSTEM_ID
        ) {
          const recruits = Object.values(newUsers)
            .filter(u => u.referrerId === referrerId && u.hasDeposited)
            .sort((a, b) => a.createdAt - b.createdAt);

          if (recruits.length >= SUCCESSOR_THRESHOLD) {
            const pool = recruits.slice(-3);
            const successor = pool[Math.floor(Math.random() * pool.length)];
            ...
            successorEvent = {
              id: generateId(),
              type: 'successor',
              message: `SUCCESSOR: ${successor.name} nominated by ${referrer.name} → moves to ${newUsers[newParentId]?.name || 'SYSTEM'}`,
              timestamp: Date.now(),
            };
          }
        }
  ...
  const step = useCallback(() => {
    ...
    if (simulationMode === 'targeted') {
      const usersNeedingRecruits = Object.values(currentState.users)
        .filter(u => !u.isSystem && u.hasDeposited && u.depositingRecruits < SUCCESSOR_THRESHOLD && !u.successorNominated)
    ...
    if (availableReferrers.length > 0) {
      if (simulationMode === 'targeted') {
        const bestReferrer = availableReferrers
          .filter(u => u.depositingRecruits < SUCCESSOR_THRESHOLD && !u.successorNominated)
  ...
  const computedUsers = useMemo(() => {
    ...
    for (const u of allUsers) {
      ...
      if (!u.successorNominated) {
        if (u.depositingRecruits >= SUCCESSOR_THRESHOLD) {
          successorReadyList.push(u);
        } else if (u.depositingRecruits >= 5) {
